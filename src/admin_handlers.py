...
async def mostrar_config(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton(
            f"🔔 Notificar admin: {'✅' if notificar else '❌'}", 
    ...
    mensaje += "Selecciona una opción para cambiarla:"
...
async def manejar_callback_config(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
# ============================================================
# ELIMINAR PREGUNTA - DESCARGAR CSV Y ELIMINAR POR NÚMERO
# ============================================================
...
async def mostrar_gestion(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, p in enumerate(preguntas, 1):
        ...
        correctas = ','.join(str(c + 1) for c in p.get('respuestas_correctas', [])) if p.get('respuestas_correctas') else ''
...
async def recibir_eliminar_pregunta(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, p in enumerate(preguntas_a_eliminar[:5], 1):
        texto_p = p.get('texto', '')[:50]
        mensaje += f"{i}. {texto_p}...\n"
    if len(preguntas_a_eliminar) > 5:
        mensaje += f"\n... y {len(preguntas_a_eliminar) - 5} más.\n"
...
class AdminHandlers:
    ...
    async def manejar_callback_admin(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Maneja callbacks de admin (redirige a la función correspondiente)"""
        query = update.callback_query
        data = query.data
